...
上边的这个图标定义，包括一个以 `_folder_dark` 为标识符的定义。
...
此外，这些关联关联中的每一个都可以针对 `light`（浅色） 和 `highContrast`（高对比度）颜色主题进行细化。
...
`light`（浅色） 和 `highContrast`（高对比度） 部分的设置属性和上面列出的文件关联设置属性完全一致，设置它们会允许在应用对应主题时，用设置的图标覆盖默认的图标。
